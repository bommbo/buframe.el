		...
		((calc (loc)
		   (let ((x 0) y)
			 ...
			 (if (display-graphic-p)
				 ;; 图形环境：使用像素坐标
				 (let* ((parent-pos (frame-position parent))
						(px (+ (car parent-pos) x))
						(py (+ (cdr parent-pos) y))
			   ...
			   (let* ((char-width (frame-char-width parent))
					  ...
					  (x-chars (/ x char-width))
					  (y-chars (/ y char-height))
					  (fw-chars (ceiling (/ (float fw) char-width)))
					  (fh-chars (ceiling (/ (float fh) char-height)))
					  (parent-width (frame-width parent))
					  (parent-height (frame-height parent))
					  (final-x (max 0 (min x-chars (- parent-width fw-chars))))
					  (final-y (max 0 (min y-chars (- parent-height fh-chars)))))
...
defaults."
  (let* ((window-min-height 1)
		 ...
		 ;; TTY 关键修改：准备 minibuffer
		 (minibuffer-param (if (display-graphic-p)
							   nil  ; GUI 环境使用 nil
							 (minibuffer-window parent-frame))))  ; TTY 必须有 minibuffer
	...
	(if (and (frame-live-p frame)
	  ...
	  ;; TTY 修改：根据环境设置 minibuffer
	  (setq frame (make-frame
				   `((name . ,frame-or-name)
					 (parent-frame . ,parent-frame)
					 (minibuffer . ,minibuffer-param)  ; TTY 关键修改
	...
	(unless (display-graphic-p)
	  (with-current-buffer parent-buffer
		(buframe--setup-tty-update-hooks)))
...
Also ensure frame is made visible."
  (let* ((frame (buframe--find frame-or-name))
		 (info (frame-parameter frame 'buframe))
	  ...
	  (with-current-buffer (plist-get info :parent-buffer)
		(if-let* ((pos (funcall fn-pos frame)))
			(pcase-let ((`(,px . ,py) (frame-position frame))
						(`(,x . ,y) pos))
			  ;; TTY 环境强制更新
			  (when (or (not (= x px)) (not (= y py))
						(not (display-graphic-p)))
				(set-frame-position frame x y)
				(when (not (display-graphic-p))
				  (redisplay t)))  ; TTY 需要强制重绘
...
;; TTY 更新钩子
(defun buframe--setup-tty-update-hooks ()
  "在终端环境下设置额外的更新钩子。"
  (unless (display-graphic-p)
	(add-hook 'window-scroll-functions
			  (lambda (_win _start)
				(run-with-idle-timer 0.05 nil 'buframe-autoupdate))
			  nil t)
